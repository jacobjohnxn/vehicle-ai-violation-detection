...
class VehicleDetectionSystem:
    def __init__(self):
        ...
        # Use custom YOLOv8 helmet model
        self.helmet_model = YOLO(r'models/customhelmet.pt')
        self.seatbelt_model = YOLO(r'models/seatbelt.pt')
        ...
        self.violation_cooldown = 2.0  # Seconds between same violation type
        self.location_threshold = 50     # Reset counter after 5 seconds
        self.helmet_classes = {
        0: 'head',
        1: 'helmet'
        }
    ...
    def check_helmet_violations(self, frame):
        ...
        for result in results[0].boxes.data:
            ...
            if confidence > 0.25:  # Adjusted threshold for this model
                if class_id == 0:  # head
                    heads.append((x1, y1, x2, y2))
                    cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 0, 255), 2)
                    cv2.putText(frame, f"HEAD ({confidence:.2f})", (x1, y1-10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                elif class_id == 1:  # helmet
                    helmets.append((x1, y1, x2, y2))
                    cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                    cv2.putText(frame, f"HELMET ({confidence:.2f})", (x1, y1-10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        
        total_riders = len(heads)
        helmet_violations = max(0, len(heads) - len(helmets))
        
        # Display detection stats
        cv2.putText(frame, f"Heads: {len(heads)}", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        cv2.putText(frame, f"Helmets: {len(helmets)}", (10, 60),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        
        if total_riders > 0:
            if total_riders > 2:
                cv2.putText(frame, "TRIPLE RIDING!", (10, 90),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                return "Triple Riding Violation", True
            if helmet_violations > 0:
                cv2.putText(frame, f"NO HELMET: {helmet_violations} rider(s)!", (10, 90),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                return f"Helmet violation - {helmet_violations} rider(s)", True
        
        return "No violation", False



    def check_seatbelt_violations(self, frame):
        results = self.seatbelt_model(frame)
        ...
        for result in results[0].boxes.data:
            ...
            if confidence > 0.6:
                ...
                elif class_id == 1:  # Seatbelt
                    cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                    cv2.putText(frame, "Seatbelt On", (x1, y1-10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        
        if persons_detected > 0 and persons_detected > len(results[0].boxes):
            seatbelt_violations = persons_detected - len(results[0].boxes)
            cv2.putText(frame, f"Seatbelt Violation: {seatbelt_violations} person(s)", (10, 140),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
            return f"Seatbelt violation - {seatbelt_violations} person(s) without seatbelt", True
        
        return "No seatbelt violation", False
    ...
    def process_frame(self, frame):
        ...
        # For tracking vehicle positions
        current_violations = []
        ...
        for result in results[0].boxes.data:
            ...
            if confidence > 0.8:
                ...
                if class_id in [2, 3, 4]:
                    violation, is_violation = self.check_helmet_violations(vehicle_roi)
                else:
                    front_seat_roi = vehicle_roi[:int(vehicle_roi.shape[0]/2), :]
                    violation, is_violation = self.check_seatbelt_violations(front_seat_roi)

                if is_violation:
                    # Check if similar violation exists nearby
                    similar_violation_exists = False
                    center_x, center_y = (x1_veh + x2_veh) // 2, (y1_veh + y2_veh) // 2

                    for prev_violation in current_violations:
                        prev_x, prev_y, prev_type = prev_violation
                        if (self.calculate_distance(center_x, center_y, prev_x, prev_y) < self.location_threshold and
                            violation == prev_type):
                            similar_violation_exists = True
                            break

                    # Check time since last violation
                    time_since_last = current_time - self.last_violation_time

                    if (not similar_violation_exists and 
                        (violation != self.last_violation or time_since_last > self.violation_cooldown)):
                        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                        ...
                        cv2.putText(frame, f"{violation}", (10, 90),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                        
                        # Update tracking
                        current_violations.append((center_x, center_y, violation))
                            ...
                            if cleaned_text.startswith(state):
                                ...
                                if len(final_plate) >= 8:
                                    ...
                                    current_time - self.processed_plates[final_plate] > 60:
                                        ...
                                        # Only proceed if model is known
...
if __name__ == "__main__":
    ...
    #system.run(r'c:\Users\jacob\OneDrive\Desktop\project needs\testimages\achutan.mp4')
    system.run(r'c:\Users\jacob\OneDrive\Desktop\project needs\testimages\tripletest.mp4')
    #system.run(r'c:\Users\jacob\OneDrive\Desktop\project needs\testimages\triberback.mp4')
